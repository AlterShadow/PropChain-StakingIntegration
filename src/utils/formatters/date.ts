...
export const formatDateByTimeStamp = (timestamp: number) => format(timestamp * 1000, 'dd/MM/yyyy');

export const formatTimeByTimeStamp = (timestamp: number) => format(timestamp * 1000, 'HH:mm a');
...
export const timePresentation = (timestamp: string, t: any) => {
...
export const getRemainTime = (dif: number) => {
	if (dif <= 0) {
		return '0 Hour 0 Min'
	}
	const hh = Math.floor(dif / 3600);
	const mm = Math.floor((dif - hh * 3600) % 60);
